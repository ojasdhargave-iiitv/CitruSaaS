...
export const findUserByEmail = async (email) => {
    const result = await pool.query('SELECT id, username, email, password, created_at FROM users WHERE email = $1', [email]);
    return result.rows[0] ?? null;
};
// Find a user by username
export const findUserByUsername = async (username) => {
    const result = await pool.query('SELECT id, username, email, password, created_at FROM users WHERE username = $1', [username]);
    return result.rows[0] ?? null;
};
...
export const createUser = async (username, email, hashedPassword) => {
    const result = await pool.query(`INSERT INTO users (username, email, password)
         VALUES ($1, $2, $3)
         RETURNING id, username, email, password, created_at`, [username, email, hashedPassword]);
    if (!result.rows[0])
